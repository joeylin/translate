...
var Chapter = require('../proxy').Chapter;
var Section = require('../proxy').Section;
...
Doc.newAndSave(docName, describe, null, null, function(err, doc) {
    console.log('  [start]');
    async.eachSeries(files, function(n, done) {
        console.log('导入%s', n);
        var f = path.resolve(ORIGIN_API_PATH, n);
        var index = files.indexOf(n);
        Chapter.newAndSave(n, index, null, doc.name, function(err, chapter) {
            fs.readFile(f, 'utf8', function(err, c) {
                if (err) return done(err);
                var plist = c.split(/\r?\n\r?\n/);
                console.log('  [%d块]', plist.length);
                ...
                async.eachSeries(plist, function(item, next) {
                    console.log('保存内容: [%s] %s', item.type, item.content.substr(0, 20));
                    var index = plist.indexOf(item);
                    Section.newAndSave(chapter._id, doc._id, item.content, null, index, function(err) {
                        if (err) {
                            return false;
                        }
                        next();
